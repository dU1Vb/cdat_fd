...
def train_and_evaluate_baseline(source_features, source_labels, target_features, device, config, evaluate_on_target=True):
    """训练并评估Baseline模型"""
    print("\n" + "="*60)
    ...
    if evaluate_on_target and target_features is not None:
        # 特征对齐（先对齐特征维度）
        _, target_features_aligned, _ = align_features(
            source_features, target_features, method='pca'
        )
        
        # 预处理目标域数据（使用已训练的scaler，但只对对齐后的特征进行处理）
        # 由于特征维度已经对齐，我们可以直接使用scaler
        target_features_processed, _ = preprocess_features(target_features_aligned, scaler=scaler, fit=False)
        
        # 创建目标域数据加载器
        target_loader = create_dataloader(
            target_features_processed, np.zeros(len(target_features_processed)),
            batch_size=config['batch_size'], shuffle=False
        )
        
        # 在目标域上评估
        print("\n[评估] Baseline模型在目标域上的表现:")
        target_metrics = evaluate_fraud_model(model, target_loader, device=device, find_optimal_threshold=False)
        print_metrics(target_metrics, prefix="  ", verbose=True)
    
    return model, scaler, source_metrics, target_metrics


def train_and_evaluate_dann(source_features, source_labels, target_features, device, config, evaluate_on_target=True):
    """训练并评估DANN模型"""
    print("\n" + "="*60)
    ...
    if evaluate_on_target:
        ...
        target_eval_loader = create_dataloader(
            target_features, np.zeros(len(target_features)),
            batch_size=config['batch_size'], shuffle=False
        )
        
        # 在目标域上评估
        print("\n[评估] DANN模型在目标域上的表现:")
        target_metrics = evaluate_fraud_model(model, target_eval_loader, device=device, find_optimal_threshold=False)
        print_metrics(target_metrics, prefix="  ", verbose=True)
    
    return model, scaler, source_metrics, target_metrics


def train_and_evaluate_finetune(source_model, target_features, target_labels, device, config):
    """在目标域上微调并评估模型"""
    print("\n" + "="*60)
    ...
    # 在源域上评估（可选）
    source_metrics = None
    if hasattr(source_model, 'encoder') and source_model is not None:
        # 预处理源域数据
        source_features, _ = preprocess_features(source_model.encoder.input_dim, scaler=scaler, fit=False)
        
        # 创建源域数据加载器
        source_loader = create_dataloader(
            source_features, np.zeros(len(source_features)),
            batch_size=config['batch_size'], shuffle=False
        )
        
        # 在源域上评估
        print("\n[评估] Fine-tune模型在源域上的表现:")
        source_metrics = evaluate_fraud_model(finetune_model, source_loader, device=device, find_optimal_threshold=False)
        print_metrics(source_metrics, prefix="  ", verbose=True)
...
def main():
    ...
    if args.mode in ['baseline', 'all']:
        baseline_model, _, _, baseline_target_metrics = train_and_evaluate_baseline(
            source_features, source_labels, target_features, device, config, args.evaluate_on_target
        )
    ...
    if args.mode in ['finetune', 'all']:
        if target_features is None or target_labels is None:
            ...
            finetune_model, _, _, finetune_target_metrics = train_and_evaluate_finetune(
                source_model, target_features, target_labels, device, config
            )
